...
SERVICES := iam mock-erp case policy communication tool-gateway
PY_PKGS := libs/recoup-common libs/recoup-erp-adapter services/iam services/mock-erp services/case services/gateway services/policy services/communication services/tool-gateway
...
logs: ## Tail service logs
	$(COMPOSE) logs -f iam mock-erp case gateway policy communication tool-gateway
...
.PHONY: help install env infra-up up down logs migrate migrate-down seed simulate-reply ingest dev-iam dev-erp dev-case dev-policy dev-comm dev-tools dev-gateway dev-web lint fmt test test-all
